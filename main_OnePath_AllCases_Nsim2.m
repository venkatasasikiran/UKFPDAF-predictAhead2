...
%             ParPath = Struct_Param_Path_Init(true,yawDevRateLoop,90,[6000,4000],400,100);
    ...
    %% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            noiseAcc = [acc;acc];
    % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
            for caseLoop = ["I","P","IE","PE"]
                if caseLoop == "P"
                    [FilterEstimate,FilterPredict,transWave.estUKF,Measurements] =UKF_PDAF3(ParPath,ParGen,Timings,Measurements,...
                                                                        noiseAcc,ClutterStruct,ParStdDev);
                    
                    XMinusVecUKF = FilterPredict';                    
                    diffBtwTrueExp2 = (ParPath.TarPathMat(Timings.impInstVec,:) - XMinusVecUKF(:,1:2)).^2;
                ...
                elseif caseLoop == "PE"
                    [FilterEstimate,FilterPredict,transWave.estUKF,Measurements] =UKF_PDAF_PE(ParPath,ParGen,Timings,Measurements,...
                                                                        noiseAcc,ClutterStruct,ParStdDev);
                    ...
                    CwIndx = find(transWave.estUKF == "C");
                    FmIndx = find(transWave.estUKF == "F");
                    trackPoints = sum(diffBtwTrueExp2(CwIndx) <= Threshold.cw) + sum(diffBtwTrueExp2(FmIndx) <= Threshold.fm);
                    trackCommInd = [FmIndx(diffBtwTrueExp2(FmIndx) <= Threshold.fm);CwIndx(diffBtwTrueExp2(CwIndx) <= Threshold.cw)];    
                ...
                switch caseLoop                    
                    case "I"
                        ...
                        tempRMS.I(nSim) = sqrt(sum(diffBtwTrueExp(trackCommInd).^2)...
                                                /length(trackCommInd));
                    case "P"
                        tempTP.P(nSim) = (trackPoints2/(impLength))*100;        
                        tempRMS.P(nSim) = sqrt(sum(diffBtwTrueExp2(trackCommInd2).^2)...
                                                /length(trackCommInd2));
                    case "IE" 
